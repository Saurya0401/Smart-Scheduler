...
class DatabaseTest(unittest.TestCase):
    ...
    def test_add_one_data(self):
        ...
        self.db.new_record(*test_data)
        retrieved_data = [self.db.fetch_record(test_data[0], query)[0] for query in
    ...
    def test_upd_one_data(self):
        ...
        for k, v in updated.items():
            self.db.update_record(test_data_id, k, v)
        retrieved_data = [self.db.fetch_record(test_data_id, query)[0] for query in
    ...
    def test_del_one_data(self):
        ...
        self.db.delete_record(test_data[0])
        retrieved_data = [self.db.fetch_record(test_data[0], query) for query in
                          (self.db.COL_STU_ID, self.db.COL_PAS_HASH, self.db.COL_SCHEDULE, self.db.COL_SUBJECTS)]
    ...
    def test_add_batch_data(self):
        ...
        retrieved_data = self.db.fetch_all_data(self.db.TAB_SUB_INFO)
